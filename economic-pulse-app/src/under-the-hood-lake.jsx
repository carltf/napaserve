...
const ARTICLE_SLUG = "lake-county-cab-2025";
...
function LivePoll({ poll }) {
  ...
  return (
    <div style={{ background: T.bg, border: `1px solid ${T.rule}`, borderRadius: 8, padding: "20px 20px 16px", marginBottom: 16 }}>
      <p style={{ fontFamily: FONTS.body, fontSize: 10, letterSpacing: "0.1em", color: T.gold, fontWeight: 700, textTransform: "uppercase", margin: "0 0 8px 0" }}>Poll</p>
      <p style={{ fontFamily: FONTS.heading, fontSize: 15, fontWeight: 700, color: T.ink, margin: "0 0 14px 0", lineHeight: 1.4 }}>{poll.question}</p>
          ...
          <div key={idx} style={{ marginBottom: 8 }}>
            {voted === null ? (
              <button onClick={() => vote(idx)} disabled={loading}
                style={{ width: "100%", textAlign: "center", background: T.surface, border: `1px solid ${T.rule}`, borderRadius: 4, padding: "9px 12px", fontFamily: FONTS.body, fontSize: 14, color: T.ink, cursor: loading ? "default" : "pointer" }}>
                {opt}
              </button>
            ) : (
              <div style={{ position: "relative", overflow: "hidden", borderRadius: 4, border: `1px solid ${isVoted ? T.accent : T.rule}` }}>
                ...
                <div style={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 12px" }}>
                  <span style={{ fontFamily: FONTS.body, fontSize: 14, color: T.ink, fontWeight: isVoted ? 600 : 400 }}>{opt}</span>
                  <span style={{ fontFamily: FONTS.body, fontSize: 13, color: T.muted, marginLeft: 12, whiteSpace: "nowrap" }}>{pct}%</span>
                </div>
              </div>
            )}
          </div>
      ...
      {voted !== null && (
        <p style={{ fontFamily: FONTS.body, fontSize: 12, color: T.muted, margin: "10px 0 0 0" }}>{total} {total === 1 ? "vote" : "votes"} · Results update in real time</p>
      )}
    </div>
  );
}
...
function PollsSection() {
  ...
  if (loading) return (
    <div style={{ padding: "24px 0", fontFamily: FONTS.body, fontSize: 14, color: T.muted }}>Loading polls...</div>
  );
  if (!polls.length) return null;

  return (
    <div style={{ borderTop: `2px solid ${T.rule}`, marginTop: 48, paddingTop: 32 }}>
      <p style={{ fontFamily: FONTS.body, fontSize: 10, letterSpacing: "0.1em", color: T.gold, fontWeight: 700, textTransform: "uppercase", margin: "0 0 6px 0" }}>Today's Polls</p>
      <h2 style={{ fontFamily: FONTS.heading, fontSize: 20, fontWeight: 700, color: T.ink, margin: "0 0 20px 0" }}>What do you think?</h2>
      {polls.map(poll => <LivePoll key={poll.id} poll={poll} />)}
      <p style={{ fontFamily: FONTS.body, fontSize: 12, color: T.muted, marginTop: 8, lineHeight: 1.5 }}>
...
function ArchiveSearch() {
  ...
  return (
    <div style={{ borderTop: `2px solid ${T.rule}`, marginTop: 48, paddingTop: 32 }}>
      <p style={{ fontFamily: FONTS.body, fontSize: 10, letterSpacing: "0.1em", color: T.gold, fontWeight: 700, textTransform: "uppercase", margin: "0 0 6px 0" }}>Archive</p>
      <h2 style={{ fontFamily: FONTS.heading, fontSize: 20, fontWeight: 700, color: T.ink, margin: "0 0 6px 0" }}>Search North Coast Coverage</h2>
      <p style={{ fontFamily: FONTS.body, fontSize: 14, color: T.muted, margin: "0 0 16px 0" }}>Search 1,000+ articles and reports from Napa Valley Features.</p>
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        <input
          ...
          style={{ flex: 1, padding: "10px 14px", fontFamily: FONTS.body, fontSize: 14, color: T.ink, background: T.surface, border: `1px solid ${T.rule}`, borderRadius: 6, outline: "none" }}
        />
        <button onClick={search} disabled={loading}
          style={{ padding: "10px 20px", background: T.accent, color: "#fff", border: "none", borderRadius: 6, fontFamily: FONTS.body, fontSize: 14, fontWeight: 600, cursor: loading ? "default" : "pointer" }}>
          {loading ? "..." : "Search"}
        </button>
      </div>

      {searched && !loading && results.length === 0 && (
        <p style={{ fontFamily: FONTS.body, fontSize: 14, color: T.muted }}>No results found. Try different keywords.</p>
      )}

      {results.map((r, i) => (
        <div key={i} style={{ borderBottom: `1px solid ${T.rule}`, padding: "14px 0" }}>
          {r.post_url ? (
            <a href={r.post_url} target="_blank" rel="noreferrer"
              style={{ fontFamily: FONTS.heading, fontSize: 15, fontWeight: 700, color: T.accent, textDecoration: "none", display: "block", marginBottom: 4 }}>
              {r.post_title || r.title || "Article"}
            </a>
          ) : (
            <p style={{ fontFamily: FONTS.heading, fontSize: 15, fontWeight: 700, color: T.ink, margin: "0 0 4px 0" }}>{r.post_title || r.title || "Article"}</p>
          )}
          <p style={{ fontFamily: FONTS.body, fontSize: 13, color: T.ink, margin: "0 0 4px 0", lineHeight: 1.5 }}>{r.chunk_text || r.text || r.content || ""}</p>
          {r.post_url && (
            <a href={r.post_url} target="_blank" rel="noreferrer"
              style={{ fontFamily: FONTS.body, fontSize: 12, color: T.muted }}>Read full article →</a>
          )}
        </div>
      ))}

      {results.length > 0 && (
        <a href="/archive" style={{ display: "inline-block", marginTop: 16, fontFamily: FONTS.body, fontSize: 14, color: T.accent, textDecoration: "underline" }}>
    ...
    <div style={s.page}>
      ...
      <div style={s.body}>
        ...
        >
          ...
          <button onClick={()=>{const canvas=chart1Ref.current;if(!canvas)return;const offscreen=document.createElement("canvas");offscreen.width=canvas.width;offscreen.height=canvas.height;const ctx=offscreen.getContext("2d");ctx.fillStyle="#FAF6F0";ctx.fillRect(0,0,offscreen.width,offscreen.height);ctx.drawImage(canvas,0,0);ctx.save();ctx.globalAlpha=0.25;ctx.font="11px 'Source Code Pro',monospace";ctx.fillStyle='#8B7355';ctx.textAlign='left';ctx.textBaseline='bottom';ctx.fillText('napaserve.org',12,offscreen.height-10);ctx.restore();const link=document.createElement("a");link.download="chart-1_weighted-avg-price_lcf_2025.png";link.href=offscreen.toDataURL("image/png");link.click();}} style={{marginTop:"8px",padding:"4px 12px",fontSize:"11px",fontFamily:"monospace",letterSpacing:"0.08em",color:"#8B7355",background:"transparent",border:"1px solid #D4C4A8",borderRadius:"3px",cursor:"pointer",display:"block"}}>DOWNLOAD CHART PNG</button>
        </ChartBlock>
        ...
        >
          ...
          <button onClick={()=>{const canvas=chart2Ref.current;if(!canvas)return;const offscreen=document.createElement("canvas");offscreen.width=canvas.width;offscreen.height=canvas.height;const ctx=offscreen.getContext("2d");ctx.fillStyle="#FAF6F0";ctx.fillRect(0,0,offscreen.width,offscreen.height);ctx.drawImage(canvas,0,0);ctx.save();ctx.globalAlpha=0.25;ctx.font="11px 'Source Code Pro',monospace";ctx.fillStyle='#8B7355';ctx.textAlign='left';ctx.textBaseline='bottom';ctx.fillText('napaserve.org',12,offscreen.height-10);ctx.restore();const link=document.createElement("a");link.download="chart-2_yoy-price-change_lcf_2025.png";link.href=offscreen.toDataURL("image/png");link.click();}} style={{marginTop:"8px",padding:"4px 12px",fontSize:"11px",fontFamily:"monospace",letterSpacing:"0.08em",color:"#8B7355",background:"transparent",border:"1px solid #D4C4A8",borderRadius:"3px",cursor:"pointer",display:"block"}}>DOWNLOAD CHART PNG</button>
        </ChartBlock>
        ...
        >
          ...
          <button onClick={()=>{const canvas=chart3Ref.current;if(!canvas)return;const offscreen=document.createElement("canvas");offscreen.width=canvas.width;offscreen.height=canvas.height;const ctx=offscreen.getContext("2d");ctx.fillStyle="#FAF6F0";ctx.fillRect(0,0,offscreen.width,offscreen.height);ctx.drawImage(canvas,0,0);ctx.save();ctx.globalAlpha=0.25;ctx.font="11px 'Source Code Pro',monospace";ctx.fillStyle='#8B7355';ctx.textAlign='left';ctx.textBaseline='bottom';ctx.fillText('napaserve.org',12,offscreen.height-10);ctx.restore();const link=document.createElement("a");link.download="chart-3_prices-by-variety_lcf_2025.png";link.href=offscreen.toDataURL("image/png");link.click();}} style={{marginTop:"8px",padding:"4px 12px",fontSize:"11px",fontFamily:"monospace",letterSpacing:"0.08em",color:"#8B7355",background:"transparent",border:"1px solid #D4C4A8",borderRadius:"3px",cursor:"pointer",display:"block"}}>DOWNLOAD CHART PNG</button>
        </ChartBlock>
        ...
        >
          ...
          <button onClick={()=>{const canvas=chart4Ref.current;if(!canvas)return;const offscreen=document.createElement("canvas");offscreen.width=canvas.width;offscreen.height=canvas.height;const ctx=offscreen.getContext("2d");ctx.fillStyle="#FAF6F0";ctx.fillRect(0,0,offscreen.width,offscreen.height);ctx.drawImage(canvas,0,0);ctx.save();ctx.globalAlpha=0.25;ctx.font="11px 'Source Code Pro',monospace";ctx.fillStyle='#8B7355';ctx.textAlign='left';ctx.textBaseline='bottom';ctx.fillText('napaserve.org',12,offscreen.height-10);ctx.restore();const link=document.createElement("a");link.download="chart-4_key-metrics_lcf_2025.png";link.href=offscreen.toDataURL("image/png");link.click();}} style={{marginTop:"8px",padding:"4px 12px",fontSize:"11px",fontFamily:"monospace",letterSpacing:"0.08em",color:"#8B7355",background:"transparent",border:"1px solid #D4C4A8",borderRadius:"3px",cursor:"pointer",display:"block"}}>DOWNLOAD CHART PNG</button>
        </ChartBlock>
