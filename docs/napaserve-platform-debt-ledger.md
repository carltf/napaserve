...
- **Status:** RESOLVED 2026-07-13 (self-hosted Protomaps vector basemap; see ADR-014)
- **Resolution:** Both explorers now load a self-hosted Napa-County Protomaps PMTiles basemap via `protomaps-leaflet` (`flavor: "light"`) at `/data/napa-basemap.pmtiles`, replacing the `tile.openstreetmap.org` raster layer — no vendor tile account, no API key. Basemap file generated by `pipeline/build_napa_basemap.sh` (bbox extract from the Protomaps daily build). vineyard-explorer → v0.3, precinct-explorer → v0.11. Vineyard is cleared to publish; precinct remains gated by PD-2026-07-07-02 only. **Live-verify gate:** the `.pmtiles` file must be generated + committed (PD-2026-07-13-02) and both pages render the basemap before this is fully closed.
- **Surfaced:** 2026-07-07
...
- **Status:** RESOLVED 2026-07-13 (Tuteur confirmed precinct→district currency; map is public)
- **Resolution:** John Tuteur (Registrar of Voters) reviewed the precinct explorer and confirmed the precinct→district assignments are current. Awaiting a *newer* Incumbent List (post-November) is a data refresh, not a validity gate. The map reads live county GIS geography only — it does NOT read the RLS-locked `napa_elected_seats` roster, so ADR-010 and the seat-verification pass (PD-2026-07-08-01) do not gate it. Precinct explorer un-gated: "demo / not for publication" language removed; card is LIVE on `/maps` + the hub, linked to `/precinct-explorer.html`. Only remaining follow-up is the Nominatim geocoder (PD-2026-07-13-01).
- **Surfaced:** 2026-07-07
...
- **Status:** RESOLVED 2026-07-13 (shared `ns-chrome.js` + `ns-chrome.css`; see ADR-015)
- **Resolution:** Factored the nav + footer into shared `public/ns-chrome.css` (styles) + `public/ns-chrome.js` (injects markup, wires drawer + newsletter). `green-library.html`, `vineyard-explorer.html` and `precinct-explorer.html` all consume it; the Green Library's inline replica + duplicate chrome CSS were removed (green-library.css 294→230 lines). One edit to `ns-chrome.js` now updates every static page. Residual: `ns-chrome.js` and `NavBar.jsx` are two definitions to hand-sync (JSX vs vanilla) — collapsing needs a build step (ADR-015).
- **Surfaced:** 2026-07-10
...
## 2026-07-13 Roll-Forward Entries — Maps page + self-hosted basemap

> **Theme:** New `/maps` landing page (React route, `napaserve-maps.jsx`) with two cards, wired into NavBar / Footer / sitemap / hub main page. Both map explorers converted from OSM community raster tiles to a self-hosted Protomaps vector basemap — **resolves PD-2026-07-07-01** (ADR-014) — and both shipped **LIVE**: the Vineyard Explorer, and the Elected Seats / precinct map (Tuteur confirmed currency → **PD-2026-07-07-02 resolved**). Standalone `public/` pages moved onto shared `ns-chrome.js` + `ns-chrome.css` — **resolves PD-2026-07-10-07** (ADR-015), with the Green Library migrated off its inline replica. A live-diagnosed **`fadeAnimation:false`** fix (protomaps-leaflet vector tiles otherwise stick at `opacity:0`) was required to make the basemap visible. Basemap file generated on Tim's Mac (`build_napa_basemap.sh`, 25 MB). All render-verified live in Chrome; code deployed.

**Roll-call (Lesson DD):** (1) PD-2026-07-07-01, -02 and PD-2026-07-10-07 resolved this session; PD-2026-07-13-02 (basemap file) generated + render-verified. (2) Basemap render verified live in Chrome on both explorers. (3) New debt below (PD-2026-07-13-01). (4) Touched the explorer/map + static-chrome subsystems (PD-2026-07-07-01/-02/-03, PD-2026-07-10-07; ADR-007/009/011/012/014/015).

#### PD-2026-07-13-01 — Precinct address search uses OSM Nominatim (now live)
- **Status:** OPEN (low severity; precinct is now public, so this is a live-site follow-up)
- **Surfaced:** 2026-07-13
- **Affected surfaces:** `precinct-explorer.html` address search (`geocodeNominatim`, `nominatim.openstreetmap.org`)
- **Symptom:** The tile blocker is resolved, but "Find an address" still calls OSM's Nominatim geocoder, which carries the same "community service, not for heavy production" caveat the tiles did.
- **Root cause:** Prototype paired Nominatim with the US Census geocoder as fallbacks.
- **Scope:** Before precinct publishes, make the US Census geocoder (no such policy) lead for all queries and treat Nominatim as a rare fallback, or drop it. The Census path already exists — flip the ordering.
- **Related entries:** PD-2026-07-07-02 (precinct publication gate); PD-2026-07-07-01 (resolved tile analogue).
- **Notes:** Not a blocker for the vineyard map (no geocoder). Only relevant when the precinct map is un-gated.

#### PD-2026-07-13-02 — `napa-basemap.pmtiles` must be generated on an open-network machine before deploy
- **Status:** RESOLVED 2026-07-13 — Tim ran `build_napa_basemap.sh` (source build `20260713`, bbox −122.70,38.10,−122.00,38.90, maxzoom 15 → 25 MB, tile type mvt, "Protomaps Basemap" schema). Basemap render verified live in Chrome on both explorers (after the fadeAnimation fix). Committed to `public/data/`. Re-run the script to refresh; if the 25 MB binary bloats history, move to R2 and repoint the `url`.
- **Surfaced:** 2026-07-13
- **Affected surfaces:** `economic-pulse-app/public/data/napa-basemap.pmtiles` (not yet generated); both explorers reference it.
- **Symptom:** Both explorers now point at `/data/napa-basemap.pmtiles`, which does not exist yet — the Cowork sandbox can't reach the Protomaps build servers (`build.protomaps.com` / `maps.protomaps.com` firewalled). Until the file exists, the maps render polygons with no basemap under them.
- **Root cause:** The basemap extract must run where the Protomaps daily build is reachable (Tim's Mac), not in the sandbox.
- **Scope:** Run `pipeline/build_napa_basemap.sh` (needs `brew install pmtiles`) on the Mac, commit the resulting file, deploy. One command.
- **Related entries:** PD-2026-07-07-01 (its live-verify gate depends on this file); ADR-014; PD-2026-07-10-06 (git image bloat — same binary-in-repo trade-off).
- **Notes:** The `.pmtiles` binary (~a few MB, county z15) commits into `public/data/`. If history bloat is a concern, host on R2 / Vercel Blob and point both explorers' `url` at that CDN.
