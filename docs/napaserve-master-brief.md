...
## New surface — The Green Library (GL-v2.3, live 2026-07-10)

**The Green Library** (`napaserve.org/green-library.html`, **GL-v2.3**) is live under **Journalism** — a tag-ribbon browse + client-side search surface over the environmental back catalog. **133 articles: 7 full-text + 126 Green Wednesday link-back cards.** A **10-tag ribbon** (garden, shade, vineyards, water-wetlands, wildlife, forests, recycling, climate, history, geology) with proximity-fade browsing; full client-side search; article-accurate hero photos from each post's Substack `og:image`. Ships as an **ADR-007 standalone `public/` page** (data in Supabase `green_library` + `green_library_tags`; not a React route), wired into the site nav/footer/hub/About with a static replica of the NapaServe chrome.

It is the **first shipped reader-facing slice toward the KB-3 semantic-search item** (the deferred Phase 3: Voyage-3 + pgvector RAG over the ~997-article corpus; the Green Library's current search is the immediate client-side tier) — and the **first of a planned collection of themed libraries** (a Library = "a curated shelf on a theme," distinct from Under the Hood's per-article analysis). **Wine** (`Wine Chronicles`, 73 posts) is #2 — content extracted, build queued; the shared-template/registry/unified-model architecture is **ADR-013**. See `napaserve-session-2026-07-10.md`.
