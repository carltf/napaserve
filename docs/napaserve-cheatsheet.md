...
## The Green Library / themed-library pattern (2026-07-10)

- **A "Library" is a standalone static page** (`public/<x>-library.html` + CSS + asset dir) that self-renders a browsable collection from Supabase via an inline `<script>` (anon publishable key, anon-read RLS on `active=true`). It is **not** a React route — Vercel serves the physical `dist/` file ahead of the SPA rewrite (same as the precinct/vineyard explorers). Distinct from Under the Hood (per-article React routes).
- **Static page in the app nav:** links to `/x-library.html` must **hard-navigate**, not client-route. `NavBar.jsx` `go()` now hard-navigates any path ending `.html` (was `/agent`-only); Footer/Hub/About use plain `<a href>` (or the About `Tool isExternal`). Four surfaces to touch: NavBar, Footer, hub, about.
- **Site chrome on a static page = hand-maintained replica.** `green-library.html` carries a static copy of `NavBar.jsx` (wordmark + hamburger drawer, 4 groups) + `Footer.jsx` (4 columns, Follow, working newsletter POST). React components can't be imported into a static page, so **it must be re-synced by hand if the app nav changes** — both files carry "keep in sync" comments. Factor into a shared snippet at library #2.
- **Article-accurate heroes via Substack `og:image`.** Every post exposes its lead photo in `<meta property="og:image" content="…">` (grep `og:image"\s*content="([^"]+)"`; the CDN URL returns a ~1200×675 jpg). Download → `sips -Z 1400 -s formatOptions 80` → save as the card's **existing filename** to avoid any DB change. Caveat: og:image = the *post* lead, so for multi-card posts it's accurate only for the **first/lead** card; give og to the lead, leave siblings on their own images (never duplicate within a post).
- **Image weight:** full-res photos are multi-MB. `sips -Z 1400 -s formatOptions 75` cut 118 heroes from ~314 MB → ~50 MB (84%). 1400 px covers the ~680 px article hero at 2× retina. (But re-committing images grows git history — see debt ledger.)
- **In-page reader back-button trap (fixed).** A hash-routed in-page reader must **pop** the entry it pushed (`history.back()`), not `pushState` a clean URL on close — the latter leaves a forward entry so the browser Back button re-opens the article. Track a `pushedArticle` flag; Back pops if we pushed, else `replaceState` to clean the URL. Cold-load deep links carry no entry to pop.
- **Ribbon slider max must be data-driven** (`tags.length - 1`), never hardcoded. Sort tags client-side for a predictable order (alphabetical), then set the default active index explicitly (e.g. the flagship tag, not index 0).
- **Green Wednesday extraction quirk:** the series has **three** summary-block formats across its run (`In "…"` / `"…" by Author, Org:` / earliest posts with no heading) — parse all three. **Wine Chronicles** (the wine equivalent, 73 posts) is **single-article** posts → 1 card each, card slug = post slug. Wine author rule: "Dan Berger's Wine Chronicles" → Dan Berger; plain "Wine Chronicles:" titles are **often Tim Carl's** → his byline, not blank.
