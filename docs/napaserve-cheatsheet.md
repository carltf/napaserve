...
## Green Library / themed-library — locked lessons (2026-07-10)

Standalone static page under `public/` (ADR-007 governs the CSS exemption; ADR-013 the library data model).

1. **`og:image` = a Substack post's LEAD photo.** Assign it only to the **lead card** of a two-article Green Wednesday post; **never duplicate the same og:image across both cards** of one post. (Extract via `og:image"\s*content="([^"]+)"`; the CDN URL returns ~1200×675 jpg. Download → `sips -Z 1400 -s formatOptions 80` → save as the card's **existing filename** so no DB change is needed.)
2. **Green Wednesday summary-block parser — three formats:** **A** (pre-Sept-2024): no "Summary of Today's Stories" heading, no pull quote; **B**: `"TITLE" by Author, Org:` (no "In" prefix); **C**: `In "TITLE," … "quote" — Author`. **Bound the summary block at the first `NAPA VALLEY, Calif.` dateline** — the naive "first standalone Share" boundary breaks because Substack embeds Share buttons between summary pairs.
3. **Author-from-deck rule (GW parsing):** the deck's "**[Name] verb**" is the writer; the **pull-quote attribution frequently names a QUOTED SOURCE, not the author**. When they disagree, **trust the deck**; confirm contributor-to-contributor flips against the parent post before locking. (Wine Chronicles: "Dan Berger's Wine Chronicles" → Dan Berger; plain "Wine Chronicles:" titles are often Tim Carl's → his byline, not blank.)
4. **Two image catalogs, distinct:** the **Green Wednesday catalog** (694 imgs, series-scoped, for the Green Library) at `~/Desktop/green-library-package/catalog-output/`; the **full NVF archive catalog** (~3,729 files, for UTH/Makers/etc., under remediation). Keep **`creator_name` / `source_organization` / `credit_line` as SEPARATE fields** — assemble the credit line per-platform at display, never bake it.

Also locked this session:
- **Static page in the app nav must hard-navigate** — `NavBar.jsx` `go()` hard-navs any `.html`; Footer/Hub/About use plain `<a href>` / `Tool isExternal`.
- **Static-page site chrome is a hand-maintained replica** of `NavBar.jsx`/`Footer.jsx` (React components can't be imported) — re-sync by hand on nav changes; factor into a shared snippet at library #2.
